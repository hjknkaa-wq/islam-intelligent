...
class FaithfulnessChecker:
    ...
    def __init__(self, threshold: float = 0.7) -> None:
        """
        Set the faithfulness threshold used to decide whether a statement is considered faithful.
        
        Parameters:
            threshold (float): Decision threshold between 0.0 and 1.0 (default 0.7). A higher value requires a higher average verification score for a statement to be considered faithful.
        """
...
class TestCitationVerifier:
    ...
    def test_verify_multiple_citations(self, temp_db: Path) -> None:
        """
        Verify that CitationVerifier correctly verifies 100 inserted evidence spans and completes within 5 seconds.
        
        Sets up a temporary SQLite database with 100 text units and corresponding evidence spans, runs verification on a statement containing 100 citations, and asserts that all citations are verified, the checked citation count equals 100, and the verification completes in under 5 seconds.
        
        Parameters:
            temp_db (Path): Path to the temporary SQLite database used for the test.
        """
        conn = sqlite3.connect(temp_db)
        try:
            conn.execute(
...
class TestFaithfulnessIntegration:
    ...
    def populated_db(self) -> None:
        """
        Create and yield a temporary SQLite database file populated with test Islamic content.
        
        The database contains tables `source_document`, `text_unit`, and `evidence_span` and is populated with one Quran verse and one Hadith entry (each with a corresponding evidence span and SHA-256 snippet hash). The fixture yields the Path to the temporary database file; the file is removed after the caller finishes (best-effort cleanup, may retry on PermissionError).
         
        Returns:
            Path: Filesystem path to the temporary SQLite database file populated for tests.
        """
