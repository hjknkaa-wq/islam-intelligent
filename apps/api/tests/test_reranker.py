...
Compute a relevance score for each input (query, document) string pair.

Parameters:
    sentence_pairs (list[tuple[str, str]]): List of (query, document) string pairs to score.

Returns:
    list[float]: A list of relevance scores in the same order as `sentence_pairs`.
"""
...
...
class CrossEncoderReranker:
    ...
    def __init__(
        ...
        """
        Initialize the reranker with the given configuration and optional model.
        
        If `config` is None, a default RerankerConfig is used. If `model` is provided, it is used directly and the reranker is marked available. If no model is provided and `config.enabled` is True, the constructor attempts to load a CrossEncoder using the configuration; on success the reranker is marked available, and if the backend is unavailable the reranker remains unavailable. If `config.enabled` is False, initialization returns early and the reranker remains disabled.
        
        Parameters:
            config (RerankerConfig | None): Reranker configuration; defaults are used when None.
            model (CrossEncoderProtocol | None): Preloaded cross-encoder implementation to use; when provided it overrides auto-loading.
        """
    ...
    def is_available(self) -> bool:
        """
        Determine whether the reranker is enabled and a usable model backend is present.
        
        Returns:
            `true` if the reranker is enabled in configuration, a model instance is set, and the backend is marked available; `false` otherwise.
        """
    ...
    def rerank(
        ...
        """
        Rerank the provided documents by relevance to the given query.
        
        If a cross-encoder model is available, returns the documents scored and ordered by the model's relevance scores, with each returned Document's score replaced by the cross-encoder score and metadata augmented with "original_score" (the prior score) and "reranked": True. If the reranker is unavailable or model prediction fails, returns the input documents sorted by their original scores in descending order.
        
        Args:
            query: The search query used for relevance scoring.
            documents: The list of Documents to rerank.
            top_k: Maximum number of documents to return; defaults to the reranker's configured top_k.
        
        Returns:
            A list of Documents ordered by relevance (highest first), truncated to at most `top_k`.
        """
    ...
    def rerank_batch(
        ...
        """
        Rerank multiple query–document sets and return the results aligned with the input queries.
        
        Each query at position i is paired with the document list at position i; if the input lengths differ, excess items are ignored. Each returned inner list contains Documents ordered by the reranker score and may be truncated according to the reranker's top_k configuration.
        
        Parameters:
            queries (list[str]): Queries to rerank.
            doc_lists (list[list[Document]]): Per-query lists of Documents to be reranked.
        
        Returns:
            list[list[Document]]: A list of reranked document lists in the same order as the paired input queries.
        """
...
def create_reranker(
    ...
    """
    Create a CrossEncoderReranker configured from the given arguments and environment.
    
    Parameters:
        enabled (bool | None): If provided, explicitly enable or disable the reranker. If `None`, the value is read from the environment variable `RAG_ENABLE_RERANKER` (accepted true values: "true", "1", "yes"; defaults to enabled when the variable is unset).
        model_name (str | None): Cross-encoder model identifier to use; defaults to "cross-encoder/ms-marco-MiniLM-L-6-v2" when `None`.
        top_k (int | None): Number of top documents the reranker will return; defaults to 10 when `None`.
    
    Returns:
        CrossEncoderReranker: A reranker instance constructed with the resolved configuration.
    """
