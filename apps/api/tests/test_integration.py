...
def db_session_integration():
    """
    Provide a fresh SQLite-backed SQLAlchemy session for integration tests.
    
    Creates the database schema if needed and yields a new Session connected to the test database. The session is closed after the caller finishes using it.
    
    Returns:
        Session: A SQLAlchemy Session connected to the freshly-initialized test database.
    """
...
def setup_cost_tables(db_session_integration):
    """
    Ensure cost governance tables exist on the provided database session.
    
    Creates the Base metadata tables and ensures the two cost-related tables
    (`cost_usage_log` and `cost_alert_event`) exist, then commits the session.
    
    Parameters:
        db_session_integration: A SQLAlchemy Session bound to the target database.
    
    Returns:
        The same `db_session_integration` session after committing the created tables.
    """
    from islam_intelligent.domain.models import Base
...
def mock_evidence_data(db_session_integration):
    """
    Insert a trusted test Source and three TextUnit records into the provided test database session for retrieval tests.
    
    Creates a Source with source_id "test_quran" and three associated TextUnit entries, commits the transaction, and returns the created TextUnit instances.
    
    Returns:
        list[TextUnit]: The three created TextUnit objects.
    """
...
class TestHyDEIntegration:
    ...
    def test_hyde_system_prompt_contains_islamic_context(self):
        """
        Verify the HyDE system prompt includes Islamic context cues.
        
        Asserts that the generated system prompt contains the word "Islam" or the word "scholar" (case-insensitive).
        """
        from islam_intelligent.rag.retrieval.hyde import HyDEQueryExpander
...
class TestCostGovernanceIntegration:
    ...
    def test_cost_governance_service_plan_query(self, setup_cost_tables):
        """
        Verify CostGovernanceService produces a valid plan for a sample query.
        
        Asserts the returned plan is allowed and includes non-None `route`, `estimate`, and `budget` fields.
        """
        from islam_intelligent.cost_governance import (
...
class TestRAGPipelineEndToEnd:
    ...
    def test_pipeline_handles_special_characters(self, monkeypatch):
        """
        Verify the RAG pipeline handles queries containing special characters without crashing.
        
        Sends several queries that include HTML/JS, single and double quotes, control characters (newlines/tabs), and SQL-like input to the pipeline and asserts each response contains `verdict` and `retrieved_count`.
        """
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        from islam_intelligent.rag.pipeline.core import RAGPipeline
