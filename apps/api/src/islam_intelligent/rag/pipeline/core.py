...
class RAGPipeline:
    ...
    def __init__(
        ...
        """
        Initialize the RAG pipeline, wiring provided components or creating sensible defaults for retrieval, expansion, reranking, generation, faithfulness verification, and cost governance.
        
        Parameters:
            config (RAGConfig | None): Pipeline configuration; a default RAGConfig is used when None.
            citation_verifier (CitationVerifier | None): Optional citation verifier to use; a default is created when None.
            generator (GeneratorProtocol | None): Optional generator (LLM) implementation; a default LLMGenerator is created from `config` when None.
            query_expander (QueryExpander | None): Optional query expander; a default QueryExpander is created based on `config` when None.
            hyde_expander (HyDEQueryExpander | None): Optional HyDE expander; a default HyDE expander is created from `config` when None.
            reranker (CrossEncoderReranker | None): Optional reranker; a default CrossEncoderReranker is created based on `config` when None.
            faithfulness_verifier (CitationFaithfulnessVerifier | None): Optional faithfulness verifier; a default is created when None.
            cost_governance (CostGovernanceService | None): Optional cost governance service. If None and cost governance is enabled in `config`, a BudgetManager and CostGovernanceService are attempted; on initialization failure the pipeline falls back with cost governance disabled and logs a warning.
        
        Notes:
            - Components passed explicitly override defaults derived from `config`.
            - If cost governance initialization fails, the constructor logs a warning and continues with cost governance disabled.
        """
    ...
    def retrieve(
        ...
        """
        Retrieve candidate evidence spans for a query, using HyDE and query-expansion/reranking options when enabled.
        
        When HyDE is enabled and available, the query may be replaced by a HyDE-expanded hypothetical query; if that happens the metadata will include a truncated `hypothetical_answer` and `hyde_used` will be true. When query expansion is enabled the metadata records the number of `query_variations` attempted.
        
        Returns:
            Tuple of `(results, metadata)` where `results` is the sequence of retrieved result mappings, and `metadata` is a dict containing at least:
              - `hyde_used` (bool): whether a HyDE expansion was applied,
              - `query_variations` (int): number of query variations used for retrieval,
              - `hypothetical_answer` (str, optional): truncated HyDE output when `hyde_used` is true.
        """
    ...
    def rerank(
        ...
        """
        Perform cross-encoder reranking of retrieved results when reranking is enabled and available.
        
        Parameters:
            query (str): The user query used to score/rerank the results.
            results (Sequence[Mapping[str, object]]): Retrieved documents or evidence items to be reranked.
        
        Returns:
            list[dict[str, object]]: The reranked list of result dictionaries; if reranking is disabled, unavailable, or fails, returns the original results converted to dictionaries.
        """
    ...
    def validate_sufficiency(
        ...
        """
        Determine whether the retrieved evidence meets the pipeline's sufficiency requirements.
        
        Parameters:
            retrieved (list[dict[str, object]]): A list of retrieved evidence items; items may include a numeric or string `score` key used to estimate quality.
        
        Returns:
            tuple[bool, float]: A pair where the first element is `True` if evidence is sufficient, `False` otherwise; the second element is a sufficiency score in the range [0.0, 1.0].
        """
        if not retrieved:
            return False, 0.0
    ...
    def apply_cost_governance(
        ...
        """
        Determine whether the pipeline should proceed with generation under configured cost governance.
        
        Returns:
            A tuple of (should_proceed, cost_estimate, degradation_message):
            - `should_proceed` (bool): `True` if the query is allowed to proceed, `False` if blocked by governance.
            - `cost_estimate` (CostEstimate | None): Estimated cost for executing the query when available, otherwise `None`.
            - `degradation_message` (str | None): Message describing any applied degradation or budget denial when present, otherwise `None`.
        
        Notes:
            On internal errors in the governance check, the method fails open and returns `(True, None, None)`.
        """
    ...
    def verify_faithfulness(
        ...
        """
        Determine whether generated statements are faithful to the retrieved evidence.
        
        Parameters:
            statements (list[Statement]): Generated statements whose claims will be checked.
            retrieved (list[dict]): Retrieved evidence items used as the source for verification.
        
        Returns:
            FaithfulnessResult: Result object containing an overall faithfulness score (0-10),
            the number of claims checked, the count of unsupported claims, per-claim details,
            and a `judge` string describing the verification pathway. If faithfulness checking
            is disabled, returns a passing result with `judge` set to "disabled". If an error
            occurs during verification, returns a passing fallback result with `judge` set to
            "error_fallback".
        """
    ...
    def generate_answer(
        ...
        """
        Produce an AnswerContract by retrieving (or using provided) evidence, validating sufficiency and trust, applying cost governance, generating statements (or using a mock), and performing citation and faithfulness verification; may abstain with an explicit reason.
        
        Parameters:
            query (str): The user query to answer.
            retrieved (list[dict[str, object]] | None): Optional pre-retrieved evidence to use instead of performing retrieval; when provided, retrieval and associated metadata are bypassed.
        
        Returns:
            AnswerContract: The pipeline result containing:
                - verdict: "answer" when statements pass all checks, or "abstain" when the pipeline stops with a reason.
                - statements: list of generated statements with citations when verdict is "answer" (empty on abstain).
                - abstain_reason / fail_reason: machine-readable reason for abstention when applicable.
                - retrieved_count: number of trusted evidence items considered.
                - sufficiency_score: computed sufficiency score for the retrieved evidence.
                - faithfulness_score: overall faithfulness score when available, otherwise None.
                - cost_usd: estimated/actual cost attributed to generation when available, otherwise None.
                - degradation_message: optional message from cost governance when behavior was degraded, otherwise None.
                - metrics: collected pipeline metrics snapshot.
        """
        ...
        def _record_ragas_scores(
            ...
            """
            Compute RAG-A-S (retrieval + generation + abstraction + sufficiency) metrics for a set of generated statements and retrieved evidence, then record those metrics with the module-level metrics collector.
            
            Parameters:
                statement_rows (Sequence[Mapping[str, object]]): Rows representing generated statements (raw generator output or normalized statement records) used to compute statement-level metrics.
                retrieved_rows (Sequence[Mapping[str, object]]): Rows representing retrieved evidence items used to compute retrieval-related metrics.
                faithfulness (float | None): Optional overall faithfulness score to include in the computed metrics (if provided).
            """
    ...
    def _generate_statements(
        ...
        """
        Produce a list of statements grounded in retrieved evidence, using the configured LLM when enabled and falling back to a deterministic mock when generation is disabled or fails.
        
        Parameters:
        	query (str): The user's query driving generation.
        	retrieved (list[dict[str, object]]): Retrieved evidence items to cite; each item is a mapping representing an evidence span and its metadata.
        
        Returns:
        	statements (list[Statement]): Statements with their associated citations constructed from the LLM output or the mock fallback.
        """
    ...
    def _verify_statements(
        ...
        """
        Verify that each generated statement contains at least one citation and that every citation's evidence_span_id appears in the retrieved results.
        
        Parameters:
            statements (list[Statement]): Generated statements to validate.
            retrieved (list[dict[str, object]]): Retrieved evidence items; each item is expected to include an `evidence_span_id` or `text_unit_id`.
        
        Returns:
            bool: `True` if all statements have at least one citation and every citation references an id present in `retrieved`, `False` otherwise.
        """
        if not statements:
            return False
    ...
    async def aretrieve(
        ...
        Returns:
            (results, metadata): `results` is a sequence of retrieved document mappings; `metadata` is a dict containing retrieval metadata (for example, hyde usage and query variations).
        """
    ...
    async def arerank(
        ...
        """
        Execute reranking for the given query and retrieved results.
        
        Returns:
            list[dict[str, object]]: Reranked results ordered by relevance.
        """
    ...
    async def agenerate_answer(
        ...
        """
        Execute the synchronous generate_answer pipeline in a worker thread so it can be awaited from async code.
        
        @return An AnswerContract containing the pipeline's verdict, statements, and associated metadata (e.g., scores, costs, and metrics).
        """
        return await asyncio.to_thread(self.generate_answer, query, retrieved)

    async def aquery(self, query: str) -> AnswerContract:
        """
        Run the full RAG pipeline for the given query and return the final AnswerContract.
        
        Returns:
            AnswerContract: Result containing the verdict ("answer" or "abstain"), statements when available, abstain or failure reason, retrieved count and sufficiency score, faithfulness score, cost estimate and degradation message, and pipeline metrics.
        """
