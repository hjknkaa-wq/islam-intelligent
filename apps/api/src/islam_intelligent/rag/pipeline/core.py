...
__all__ = [
    ...
    def _execute_retrieval_with_metrics(
        self,
        query: str,
        provided_retrieved: list[dict[str, object]] | None,
        metrics_collector: MetricsCollector,
    ) -> tuple[list[dict[str, object]], dict[str, object]]:
        """Execute retrieval step and record metrics.
        
        Args:
            query: The search query
            provided_retrieved: Optional pre-retrieved evidence
            metrics_collector: Metrics collector instance
            
        Returns:
            Tuple of (retrieved_results, metadata)
        """
        if provided_retrieved:
            # Use provided evidence (backward compatibility)
            raw_retrieved_list = [dict(r) for r in provided_retrieved]
            retrieval_metadata = {"hyde_used": False, "query_variations": 1}
            metrics_collector.record_retrieval(
                results=raw_retrieved_list,
                latency_ms=0.0,
            ...
            return raw_retrieved_list, retrieval_metadata
        ...
        if (
            ...
            raw_retrieved_obj = retrieve_result[0]
            retrieval_metadata = dict(cast(Mapping[str, object], retrieve_result[1]))
        else:
            raw_retrieved_obj = retrieve_result
            retrieval_metadata = {"hyde_used": False, "query_variations": 1}
        ...
        metrics_collector.record_retrieval(
            ...
            latency_ms=retrieval_latency,
            hyde_used=bool(retrieval_metadata.get("hyde_used")),
            query_variations=int(str(retrieval_metadata.get("query_variations", 1))),
            reranker_used=False,  # Will update after reranking
        )
        
        return raw_retrieved_list, retrieval_metadata

    def _execute_reranking_with_metrics(
        self,
        query: str,
        raw_retrieved: list[dict[str, object]],
        metrics_collector: MetricsCollector,
    ) -> list[dict[str, object]]:
        """Execute reranking step and record metrics.
        
        Args:
            query: The search query
            raw_retrieved: Raw retrieval results
            metrics_collector: Metrics collector instance
        ...
        """
        metrics_collector.start_stage("reranking")
        reranked = self.rerank(query, raw_retrieved)
        reranking_latency = metrics_collector.end_stage("reranking")
    ...
    def _build_abstention_response(
        ...
        abstain_reason: str,
        fail_reason: str,
        trusted_retrieved: list[dict[str, object]],
        sufficiency_score: float,
        faithfulness_score: float | None,
        cost_usd: float,
        degradation_message: str | None,
        metrics_collector: MetricsCollector,
        record_ragas_fn: callable,
    ) -> AnswerContract:
        """Build abstention response with metrics."""
        record_ragas_fn(statement_rows=[], retrieved_rows=trusted_retrieved)
        metrics_collector.record_verdict(
            verdict="abstain", abstain_reason=abstain_reason
        )
        metrics_collector.finalize_and_record()
    ...
    def _build_success_response(
        ...
        """Build successful answer response with metrics."""
        metrics_collector.record_verdict(verdict="answer")
        metrics_collector.finalize_and_record()
